// In line with sharing the header and footer between all pages,
// I have also moved all javaScript into one file.
// It's probably poorly optimized but it works sooooooo it is what it is
// - Robert

// NOTE: Please check elements existence before attempting to use to prevent script from dying.
                ...
                <div class="flex-1 flex items-center justify-end">
                    ...
                    <div class="flex items-center gap-3 lg:gap-4 ml-3 lg:ml-6">
                        ...
                        <button id="nav-wellness-pill" type="button" class="hidden md:inline-flex items-center gap-2 px-3 py-2 rounded-full bg-apple-panel/90 border border-white/10 text-apple-dark hover:bg-apple-panel transition-colors shadow-sm">
                            ...
                            <span id="nav-wellness-count" class="min-w-[1.5rem] text-center text-xs font-bold px-2 py-0.5 rounded-full bg-white/80 border border-apple-dark/10">
                                0
                            </span>
                        </button>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-12 text-center md:text-left">
                ...
                <div>
                    ...
                    <ul class="space-y-2 text-gray-400 text-sm">
                        ...
                        <!-- <li><a href="index.html#about" class="hover:text-apple-red transition-colors">About</a></li> -->
...
// Listener for shared page content
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Render Icons with header & footer
    if (window.lucide) {
        lucide.createIcons();
    }
});

// // Render Icons - Shouldn't be necessary since they render with the header/footer
// if (window.lucide) {
//     lucide.createIcons();
// }
...
function moveSlide(carouselId, direction) {
    ...
    if (carouselStates[carouselId] >= totalSlides) {
        carouselStates[carouselId] = 0;
    } else if (carouselStates[carouselId] < 0) {
        carouselStates[carouselId] = totalSlides - 1;
    }
    
    // Calculate and apply the transform percentage (-100% per slide)
    const slideOffset = -(carouselStates[carouselId] * 100);
...
if (eventMapEl && typeof L !== 'undefined') {
...
// Swiper Script
const swiper = new Swiper('.sub-container.swiper', {
    loop: true,
    spaceBetween: 20,
    pagination: {
...
// Whistle-Stop Wellness (isolated feature)
(() => {
    ...
    if (!cards.length || !resultTitle || !resultSteps || !resultWhy || !btnBank || !btnCopy || !btnShuffle || !streakEl) return;
    ...
    const setSelected = (key) => {
        ...
        // Keep icons in newly inserted HTML consistent
        if (window.lucide) {
            lucide.createIcons();
        }
    };
    ...
    const copySteps = async () => {
        ...
        try {
            ...
            // Fallback: select-to-copy prompt
    ...
    if (navPill) {
        navPill.addEventListener('click', () => {
            document.getElementById('wellness')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
            // If nothing is selected yet, pick a default to make the click feel “active”
            if (!selectedKey) setSelected('move');
        });
    }
